...
export class FileRetrieveService {
    ...
    async getFileName(fildId: string) {
        const res = await this.fileManagerModel.findById(fildId);
        if (res.state === "No") {
            throw new NotFoundException();
        }
    ...
    async getLuotXem(): Promise<any> {
        return await this.fileManagerModel.find({}, "filename luotXem luotTai");
    }
}
